...
export class AccountPool {
  ...
  mark429(id, errorCode) {
    ...
    if (errorCode === 4006) {
      ...
      // Use cooldown (not pin to cap) so account retries after capacity returns.
      const until = now + this.cooldown429;
      this._mark429.run(until, id);
      const remaining = this._countAvailable();
      this.log.info?.(`429 -> account #${id} capacity-4006 -> cool ${this.cooldown429}s, ${remaining} remaining`);
      return;
    }
    ...
    this._mark429.run(until, id);
    const remaining = this._countAvailable();
    this.log.info?.(`429 -> account #${id} rate-limit -> cool ${this.cooldown429}s, ${remaining} remaining`);
  }
