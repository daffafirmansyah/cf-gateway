...
export class AccountPool {
  constructor(db, { cooldown429 = 90, log = console, reserveNeurons = 250 } = {}) {
    ...
    this.CAPACITY_GATE_MS = 90000;   // 90s gate cooldown (protect IP from bans)
  ...
  checkCapacityGate() {
    ...
    if (now < this._capacityGateUntil) {
      ...
      return { blocked: true, waitMs, reason: `global capacity gate active (${Math.ceil(waitMs / 1000)}s remaining)` };
    }
  ...
  markCapacityHit() {
    ...
    if (this._capacityHits.length >= this.CAPACITY_THRESHOLD) {
      ...
      this.log.warn?.(`GLOBAL CAPACITY GATE ACTIVATED — ${this._capacityHits.length} capacity errors in ${this.CAPACITY_WINDOW_MS / 1000}s — pausing all retries for ${this.CAPACITY_GATE_MS / 1000}s`);
