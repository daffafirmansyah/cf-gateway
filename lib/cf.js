...
export async function callNormal(url, apiKey, body, { timeoutMs = 30000 } = {}) {
 ...
 */
export async function openStream(url, apiKey, body, { timeoutMs = 60000 } = {}) {
  const res = await fetch(url, {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
 ...
 */
export async function pumpStream(stream, write) {
